...
public class MyFormAuthenticationFilter extends AuthenticatingFilter {
    ...
    protected boolean onLoginSuccess(AuthenticationToken token, Subject subject,
        ...
        userLoginExample.createCriteria().andUsernameEqualTo(username);
        List<UserLogin> userLogins = userLoginService.selectByExample(userLoginExample);
        UserLogin user = userLogins.get(0);

        UserLogin userLogin = new UserLogin();
        userLogin.setLastIp(ParamUtils.getIpAddr((HttpServletRequest)request));
        userLogin.setLastTime(new Date());
        userLogin.setLoginCount(user.getLoginCount()+1);
        userLoginService.updateByExampleSelective(userLogin, userLoginExample);
        //更新字段缓存
        FieldCacheManager.reloadFieldCache();
