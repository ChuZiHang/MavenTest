...
public class RequestParser {
	...
	private String customFilename;//自定义文件名称规则
	...
	static RequestParser getInstance(HttpServletRequest request, MultipartUploadParser multipartUploadParser)
		...
		if (multipartUploadParser == null) {
			...
			// params could be in body or query string, depending on Fine
			// Uploader request option properties
			parseRequestBodyParams(requestParser, multipartUploadParser);
			parseQueryStringParams(requestParser, request);
		}
		//文件名=uuid+扩展名
		String fileSuffix = requestParser.getFilename().substring(requestParser.getFilename().lastIndexOf("."));
		requestParser.customFilename = requestParser.getUuid()+fileSuffix;
	...
	private static void parseRequestBodyParams(RequestParser requestParser, MultipartUploadParser multipartUploadParser)
			throws Exception {
		if (multipartUploadParser.getParams().get(GENERATE_ERROR_PARAM) != null) {
			requestParser.generateError = Boolean
					.parseBoolean(multipartUploadParser.getParams().get(GENERATE_ERROR_PARAM));
		}
