...
<html lang="zh-CN">
	...
	<body class="baseBgColor">
		    ...
		    <div class="branch_wrap_cen">
		        ...
		        <div class="rightContent">
		            ...
		            <c:if test="${users!=null && fn:length(users)>0}">
			            <c:forEach items="${users}" var="userMap" begin="0" end="0">
			            	<c:set var="custInfo" value="${userMap.key}"></c:set>
			            	<div class="salesmanBox">
				                <div class="salesman-tag" style="background: none">总部 
				                	<span class="salesman-toggle">收起</span>
				                	<shiro:hasPermission name="settings/user:add">
				                    	<div class="joinsalesman j-zong" data-custId="${custInfo.systemId}" data-custName="总部">添加业务员</div>
				                	</shiro:hasPermission>
				                </div>
				                <div class="member_item_table" style="display: block;">
		            ...
		            <c:if test="${users!=null && fn:length(users)>0}">
			            <c:forEach items="${users}" var="userMap" begin="1">
			            	<c:set var="custInfo" value="${userMap.key}"></c:set>
			            	<div class="salesmanBox">
				                <div class="salesman-tag">
				                    ${custInfo.custName}<span class="salesman-toggle">展开</span>
				                    <shiro:hasPermission name="settings/user:add">
				                    	<div class="joinsalesman j-zong" data-custId="${custInfo.systemId}" data-custName="${custInfo.custName}">添加业务员</div>
				                	</shiro:hasPermission>
				                </div>
				                <div class="member_item_table" style="display: none;">
									<table cellpadding="0" cellspacing="0" border="0" width="100%" class="lis-table" id="ul_${custInfo.systemId}">
										<tr>
											<th bgcolor="#f3f5f6" height="36">账号</th>
											<th bgcolor="#f3f5f6" height="36">姓名</th>
		...
		<div class="joinsalesmanBox closebox">
		    <h3 class="create_syllabus_title">添加业务员（<span id="targetCust">总部</span>）</h3>
